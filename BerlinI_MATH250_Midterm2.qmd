...
> According to Gallup, 3.4% of U.S. adults identify as gay and 5.2% identify as bisexual. Assume the remaining 91.4% are straight (which isn’t really true). Is the `cupid` data set consistent with this information? Follow all best practices from class.
...
```
There is a $2.2\times10^{-16}$ chance of getting data like the sample data above if the ratios reported by Gallup are accurate. This minuscule chance does not support Gallup's claim. The data from OkCupid is not consistent with the claims made by Gallup. 
